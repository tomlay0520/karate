...
def excel_to_db(excel_file_path, db_file_path):
    """
    把 excel 文件里的运动员信息导入到数据库
    """
    try:
        excel_file = pd.ExcelFile(excel_file_path)
        df = excel_file.parse('运动员汇总表')
        df.columns = ['单位名称', '姓名', '性别', '身份证', '身高/体重', '组  别_分量制', '组  别_型', '组  别_无差', '备注']  # pyright: ignore[reportAttributeAccessIssue]
        df = df[3:]  # 跳过前3行（如有表头或说明）  # pyright: ignore[reportArgumentType]
        ...
        for _, row in df.iterrows():
            if pd.isnull(row['姓名']) or row['姓名'] == '':  # pyright: ignore[reportUnknownMemberType, reportGeneralTypeIssues]
                continue

            insert_query = """
                INSERT OR IGNORE INTO ath (
                    unit_name, ath_name, gender, id_card, weight, 
                    weight_catogory, kata, open, remarks
                ) VALUES (?,?,?,?,?,?,?,?,?);
            """
